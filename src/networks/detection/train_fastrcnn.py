...
class DetectNet:
    ...
    def train(self, dataset_path, dataset_val_path, name):
        ...
        data_loader = torch.utils.data.DataLoader(
            dataset, batch_size=10, shuffle=True, num_workers=16, collate_fn=collate_fn
        )
        ...
        for epoch in range(n_epochs):  # Training loop
            ...
            for batch_idx, (images, targets) in enumerate(data_loader):
                ...
                if (batch_idx > 0 and batch_idx % eval_freq == 0) or batch_idx == len(
                    data_loader
                ) - 1:
                    val_loss = self.evaluate(data_loader_test)
                    print(
                        f"(batch {total_batch_index}) Train: {total_loss / total_items:.4f}   -   Val: {val_loss:.4f}"
                    )
    ...
    def test(self, modelPath, testFolder):
        ListImages = glob.glob(
            os.path.join(testFolder, "*.jpg")
        ...
        for imgPath in ListImages:
            ...
            if img.shape[0] > max_height:
                img = cv2.resize(
                    img, (int((max_height / img.shape[0]) * img.shape[1]), max_height)
                )
            inputs.append(img)
        ...
        for img_path, img, output in zip(ListImages, inputs, outputs):
            ...
            cv2.imwrite(
                f"/workspace/CL/output/{Path(img_path).stem}_detect.jpg",
                img.astype(np.int32),
            )
...
if __name__ == "__main__":
    Net = DetectNet(
        train=True, device=f"cuda:{DEVICE}"
    )  # , pretrained_path="/workspace/ChessLink/model/detection/latest.torch")
    # Net.visualize_train()
    # Net.test("/workspace/CL/model/detection/5.torch", "/workspace/CL/data/test_images")
    Net.train(
        "/workspace/ChessLink/data/dataset_yolo_merge/train",
        "/workspace/ChessLink/data/chessred_test_yolo",
        name="train2",
    )
