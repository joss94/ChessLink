...
class YOLOv8ONNX:
    ...
    def postprocess(self, input_image, output):
        ...
        return (
            np.array([boxes[i] for i in indices]),
            np.array([class_ids[i] for i in indices]),
            np.array([scores[i] for i in indices]),
        )
    ...
    def __call__(self, image):
        ...
        boxes, labels, scores = self.postprocess(self.img, outputs)  # output image
