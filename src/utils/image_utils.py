...
def draw_board_on_image(board, image, highlights=[]):
    ...
    board_img = get_board_image(board, board_size, highlights)
...
def get_board_image(board, board_size, highlights=[]):
    svg = chess.svg.board(
        board, fill=dict.fromkeys(highlights, "#cc0000cc"), size=board_size
    )
...
def draw_squares_on_image(image, squares_corners):
    cv2.polylines(
        image, squares_corners, True, (255, 0, 0), max(1, int(image.shape[0] / 500))
    )
