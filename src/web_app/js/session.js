...
const liveBoardElement = document.getElementById("live-board");
...
function startStream() {
  ...
  if (streamType == "Camera") {
    ...
    const stream_url = urlParams.get("stream_url");
    startURLStream(stream_url);
  } else if (streamType == "File") {
    const video_file = urlParams.get("video_file");
    startFileStream(video_file);
  }
}
...
async function CameraCallback() {
  let width = liveVideoElement.width;
  let height = liveVideoElement.height;

  if (width != 0 && height != 0) {
    //liveVideoElement.width = width;
    //liveVideoElement.height = height;
...
function startURLStream(stream_url) {
  liveVideoElement.style.display = "none";
  liveImgElement.style.display = "block";
  liveImgElement.src = stream_url;
...
function detectMotion() {
  ...
  if (board_mask != null && last_frame != null) {
    let move_count = compute_motion_mask(
      frame,
      last_frame,
      board_mask,
      board_rect
  ...
  if (
    still == STILL_TIME ||
    last_board_det_time == null ||
    Date.now() - last_board_det_time > 30000
  ) {
...
function processFrame() {
  ...
  myObject['width'] = frame.cols;
  myObject['height'] = frame.rows;
  myObject['original_width'] = liveVideoElement.videoWidth
  myObject['original_height'] = liveVideoElement.videoHeight
  myObject['session_id'] = session_id;
  myObject['frame'] = Array.from(frame.data);
  ...
  fetch(getURL() + "/process_frame", {
    ...
    .then((json_rsp) => {
      ...
      if (json_rsp.status != "ok") {
        throw new Error(
          "Server returned an invalid status: " + json_rsp.status
        );
      }
