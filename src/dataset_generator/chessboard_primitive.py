            ...
            if mod.type == "NODES":
                for input in [
                    ...
                    if input.socket_type == "NodeSocketFloat":
                        mod[input.identifier] = np.random.uniform(
                            input.min_value, input.max_value
                        )
                    elif input.socket_type == "NodeSocketInt":
                        mod[input.identifier] = np.random.randint(
                            input.min_value, input.max_value
                        )
            ...
            for mat in piece.data.materials:
                for n in mat.node_tree.nodes:
                    if n.type == "GROUP":
                        for input in [
                            s
                            for s in n.node_tree.interface.items_tree
                            if s.in_out == "INPUT"
                        ]:
                            if input.socket_type == "NodeSocketFloat":
                                n.inputs[
                                    input.identifier
                                ].default_value = np.random.uniform(
                                    input.min_value, input.max_value
                                )
                            elif input.socket_type == "NodeSocketInt":
                                n.inputs[
                                    input.identifier
                                ].default_value = np.random.randint(
                                    input.min_value, input.max_value
                                )
                            elif input.socket_type == "NodeSocketBool":
                                n.inputs[input.identifier].default_value = (
            ...
            for i in range(random.randint(min_qty, max_qty)):
                ...
                if piece_obj:
                    ...
                    piece_obj.location = (
                        np.array(
                            [
                                0.5 + file + random.uniform(-0.2, 0.2),
                                rank + 0.5 + random.uniform(-0.2, 0.2),
                                0,
                            ]
                        )
                        * board_size
